...
class ResBlock(nn.Module):
    ...
    def forward(self, x, get_x_bn=False, get_x_bn_relu=False):

        branch2_ = self.bn_branch2a(x)
        x_bn=branch2_
        branch2 = F.relu(branch2_)
    ...
    def __call__(self, x, get_x_bn=False, get_x_bn_relu=False):
        return self.forward(x, get_x_bn=get_x_bn, get_x_bn_relu=get_x_bn_relu)
...
class Net(nn.Module):
    ...
    def forward_as_dict(self, x):
        ...
        x, conv3 = self.b4(x,get_x_bn=True)
        x = self.b4_1(x)
        x = self.b4_2(x)
        x = self.b4_3(x)
        x = self.b4_4(x)
        ...
        x, conv4 = self.b5(x, get_x_bn=True)
        x = self.b5_1(x)
        x = self.b5_2(x)

        x, conv5 = self.b6(x, get_x_bn=True)

        xb7, conv6 = self.b7(x,get_x_bn=True)

        conv7 = F.relu(self.bn7(xb7))

        return dict({ 'conv3':conv3,'conv4': conv4, 'conv5': conv5,'conv6': conv6, 'conv7': conv7,'xb7': xb7})
